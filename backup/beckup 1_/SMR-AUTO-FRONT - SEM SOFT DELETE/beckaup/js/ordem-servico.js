document.addEventListener('DOMContentLoaded', () => {
    ...
    async function carregarPecas() {
        ...
        try {
            ...
            if (response.ok && data) {
                ...
                console.log(`Carregadas ${data.length} peças para os selects`);
            } else {
    ...
    async function buscarOrdens() {
        ...
        // Implementar busca por cliente, veículo ou ID da ordem
        try {
            const { response, data } = await fetchApi('/ordem-servico');
            console.log("Resposta da API para busca:", response.status, data ? data.length : 0);
            
            if (response.ok && data) {
                // Filtrar ordens pelo termo de busca (cliente ou veículo)
                const ordensFiltradas = data.filter(ordem => {
                    try {
                        const clienteNome = ordem.cliente && ordem.cliente.nmCliente 
                            ? ordem.cliente.nmCliente.toLowerCase() : '';
                        const veiculo = ordem.veiculo ? ordem.veiculo.toLowerCase() : '';
                        const id = ordem.idServico ? ordem.idServico.toString() : '';
                        
                        return clienteNome.includes(searchTerm.toLowerCase()) || 
                               veiculo.includes(searchTerm.toLowerCase()) ||
                               id === searchTerm;
                    } catch (e) {
                        console.error("Erro ao filtrar ordem:", e, ordem);
                        return false;
                    }
                });
                
                console.log(`Encontradas ${ordensFiltradas.length} ordens`);
    ...
    function adicionarCampoPeca() {
        ...
        // Carregar as opções de peças para o novo select
        carregarPecas();
    ...
    // Função para renderizar ordens que estava faltando
    ...
    async function editarOrdem(id) {
        ...
        try {
            ...
            if (response.ok && data) {
                ...
                // Carregar as opções de peças
                await carregarPecas();
                
                // Selecionar as peças corretas após carregar as opções
                if (Array.isArray(data.pecasUtilizadas) && data.pecasUtilizadas.length > 0) {
                    data.pecasUtilizadas.forEach((pecaServico, index) => {
                        if (!pecaServico || !pecaServico.peca) {
                            return;
                        }
                        
                        const select = document.getElementById(`peca-${index + 1}`);
                        if (select && pecaServico.peca && pecaServico.peca.idPeca) {
