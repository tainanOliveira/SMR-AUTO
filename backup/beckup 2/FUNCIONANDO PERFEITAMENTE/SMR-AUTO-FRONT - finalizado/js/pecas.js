document.addEventListener('DOMContentLoaded', () => {
    ...
    // Função de verificação de estoque baixo corrigida
async function mostrarEstoqueBaixo() {
    try {
        console.log("Verificando peças com estoque baixo...");
        const { response, data } = await fetchApi('/peca/estoque-baixo');
        
        if (response.ok && data) {
            console.log("Peças com estoque baixo recebidas:", data);
            
            if (data.length > 0) {
                renderizarPecas(data);
                mostrarAlerta(`${data.length} peças com estoque baixo encontradas`, 'warning');
            } else {
                mostrarAlerta('Nenhuma peça com estoque baixo', 'success');
                carregarPecas();
            }
        } else {
            console.error("Erro na resposta da API (estoque baixo):", response.status);
...
// Função de ajuste de estoque corrigida
async function ajustarEstoque(id, nome, estoqueAtual) {
    ...
    stockForm.addEventListener('submit', async (e) => {
        ...
        try {
            console.log(`Ajustando estoque da peça ${id}. Nova quantidade: ${novaQuantidade}, Diferença: ${diferenca}`);
            ...
            if (response.ok) {
                ...
                console.error("Erro ao atualizar estoque: estoque insuficiente");
